...
export function menuGenerico() {
  ...
  do {
    ...
    if (!errorIngreso) {
      lineaConRecuadroError(
        30,
        "Opción inválida. Por favor, reinteinte",
        40,
        2
      );
    }
    ...
    opcion = rls.question(
      igualoCadena("", 31, " ") + "Seleccione una de las opciones:".green
    );
...
export function pantallaMenu(
  ...
  opciones.forEach((element) => {
    armaLinea(padIzquierdo, element, ancho, set);
  });
  cierreInferior(padIzquierdo, ancho, set);
}

// arma linea superior de cuadro
export function cierreSuperior(
  padIzquierdo: number,
  ancho: number,
  set: number
) {
...
// arma linea inferior de cuadro
export function cierreInferior(
  padIzquierdo: number,
  ancho: number,
  set: number
) {
...
export function lineaConRecuadro(
  padIzquierdo: number,
  texto: string,
  ancho: number,
  set: number
...
// arma las lineas interna de un cuadro
export function armaLineaError(
  padIzquierdo: number,
  texto: string,
  ancho: number,
  set: number
...
export function lineaConRecuadroError(
  padIzquierdo: number,
  texto: string,
  ancho: number,
  set: number
...
export function validarNumeroEntre(
  cartel: string,
  min: number,
  max: number,
  averificar: number
  ...
  while (true) {
    ...
    numero = rls.questionInt(
      igualoCadena("", 31, " ") + "Ingrese el valor: ".green
    );
...
export function ingresarString(
  cartel: string,
  cad1: string,
  cad2: string,
  cad3?: string
  ...
  do {
    ...
    cadena = rls.question(
      igualoCadena("", 31, " ") + `Ingrese  ${cartel} : `.green
    );
